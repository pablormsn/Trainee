...
CREATE TABLE IF NOT EXISTS `Trainee`.`Serie` (
  ...
  `Ejercicio_Musculo_idMusculo` INT NOT NULL,
  PRIMARY KEY (`idSerie`, `Ejercicio_idEjercicio`, `Ejercicio_Musculo_idMusculo`),
  INDEX `fk_Serie_Ejercicio1_idx` (`Ejercicio_idEjercicio` ASC, `Ejercicio_Musculo_idMusculo` ASC) VISIBLE,
  CONSTRAINT `fk_Serie_Ejercicio1`
    FOREIGN KEY (`Ejercicio_idEjercicio` , `Ejercicio_Musculo_idMusculo`)
    REFERENCES `Trainee`.`Ejercicio` (`idEjercicio` , `Musculo_idMusculo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
